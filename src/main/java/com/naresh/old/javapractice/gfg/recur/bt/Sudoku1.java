package com.naresh.old.javapractice.gfg.recur.bt;
...
public class Sudoku1 {
	...
	private static void solveSudoku(char[][] input) {
		...
		solveSodokuUtil(input, horizontal, vertical, box, 0, 0);
	}

	private static boolean solveSodokuUtil(char[][] input, boolean[][] horizontal, boolean[][] vertical, boolean[][] box,
			int row,int col) {
		if(col == 9){
			row = row + 1;
			col = 0;
		...
		if(input[row][col] != '.'){
			return solveSodokuUtil(input, horizontal, vertical, box, row, col+1);
		}
		for(int val =1; val <=9; val++){
			int index = 3*(row/3) + col/3;
			if(horizontal[row][val - 1] ==false && vertical[col][val -1] == false){
				horizontal[row][val -1] = true;
				vertical[col][val -1] = true;
				box[index][val -1] = true;
				input[row][col] = (char)(val + '0');
				
				if(solveSodokuUtil(input, horizontal, vertical, box, row, col+1)){
					return true;
				}
