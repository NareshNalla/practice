...
*   **Talking about Different GC Collectors:**
    ...
    -   **ZGC / Shenandoah:** The state-of-the-art. These are ultra-low-latency collectors designed for massive heaps (multi-terabyte) and applications that require pauses of less than a millisecond. They do almost all work concurrently. Mentioning these shows you are up-to-date with the latest JVM developments. Default Status: ZGC is not the default , needs to enable-XX:+UseZGC
